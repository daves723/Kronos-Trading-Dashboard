...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, q, k, position_offset=0):
        total_len = q.shape[-2] + position_offset
...
class MultiHeadAttentionWithRoPE(nn.Module):
    ...
    def forward(self, x, key_padding_mask=None, past_kv=None, position_offset=0):
        ...
        if use_cache:
            device = x.device
            q_pos = torch.arange(position_offset, position_offset + seq_len, device=device)
        ...
        if key_padding_mask is not None:
            if use_cache:
                ...
                past_padding = key_padding_mask.new_zeros(batch_size, past_len)
                full_padding = torch.cat([past_padding, key_padding_mask], dim=1)
            else:
                full_padding = key_padding_mask
            pad_mask = full_padding.unsqueeze(1).unsqueeze(2).expand(-1, self.n_heads, seq_len, -1)
...
class MultiHeadCrossAttentionWithRoPE(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, past_kv=None, position_offset=0):
        ...
        # q always at position 0; k at position_offset..position_offset+seq_len-1
        q, _ = self.rotary(q, q.clone(), position_offset=0)

        use_cache = past_kv is not None
        if use_cache:
            past_k, past_v = past_kv
            _, k = self.rotary(k.clone(), k, position_offset=position_offset)
            k = torch.cat([past_k, k], dim=-2)
            v = torch.cat([past_v, v], dim=-2)
        else:
            _, k = self.rotary(k.clone(), k, position_offset=0)
        ...
        if key_padding_mask is not None:
            if use_cache:
                ...
                full_padding = key_padding_mask
            attn_mask = full_padding.unsqueeze(1).unsqueeze(2).expand(-1, self.n_heads, q_len, -1)
        else:
            attn_mask = None
...
class DependencyAwareLayer(nn.Module):
    ...
    def forward(self, hidden_states, sibling_embed, key_padding_mask=None, past_kv=None, position_offset=0):
        """hidden_states: [batch, seq_len, d_model]
        sibling_embed: Embedding from another subtoken
        """
        use_cache = past_kv is not None
        attn_result = self.cross_attn(
            query=sibling_embed,
            key=hidden_states,
            value=hidden_states,
        ...
        if use_cache:
            ...
            return self.norm(hidden_states + attn_out), new_kv
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, key_padding_mask=None, past_kv=None, position_offset=0):
        ...
        residual = x
        x = self.norm1(x)
        attn_result = self.self_attn(x, key_padding_mask=key_padding_mask,
